<section class="grid-statistik">
    @foreach($kartu as [$label, $nilai, $ikon, $warna])
        <article class="kartu-statistik">
            <span class="ikon-statistik {{ $warna }}">
                <svg><use href="#{{ $ikon }}"/></svg>
            </span>
            <div>
                <small>{{ $label }}</small>
                <strong>{{ number_format($nilai, 0, ',', '.') }}</strong>
            </div>
        </article>
    @endforeach
</section>
<section class="panel">
    <div class="judul-panel">
        ...
        <a href="{{ route('arsip') }}">Lihat semua</a>
    </div>
    <div class="bungkus-tabel">
        <table>
            ...
            <tbody>
                ...
                @empty
                    <tr><td colspan="6" class="kosong">Belum ada dokumen.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<section class="panel ringkasan">
    ...
    @php
        $maks = max(1, ...array_column($kartu, 1));
    @endphp
    <div class="area-grafik">
        <div class="grafik-batang" role="img" aria-label="Grafik jumlah arsip">
            @foreach(array_slice($kartu, 0, 4) as [$label,$nilai])
                <div class="batang-item">
                    <span class="batang" style="--tinggi: {{ max(5, round(($nilai/$maks)*100)) }}%">
                        <i>{{ $nilai }}</i>
                    </span>
                    <small>{{ $label }}</small>
                </div>
        ...
        <ul class="legenda">
            @foreach(array_slice($kartu, 0, 4) as [$label,$nilai,$ikon,$warna])
                <li><i class="titik {{ $warna }}"></i> {{ $label }} <strong>{{ $nilai }}</strong></li>
            @endforeach
        </ul>
    </div>
</section>
